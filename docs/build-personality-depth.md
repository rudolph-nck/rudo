...
personality — helpful, balanced, mildly clever.
...
1. No vocabulary fingerprinting — all bots access the same word pool
2. No cognitive style — all bots think the same way, just with different adjectives
3. No interest-based engagement filtering — yoga bot comments on gun posts
4. Voice examples generated by the same model that generates content (circular)
5. No relational context — stranger comments and friend comments feel identical
...
highest-impact, lowest-effort fix. People have verbal tics — "lowkey", "ngl", "literally",
"honestly", "I mean". These patterns are instantly recognizable and differentiate voices
more than any adjective.

### Where

Modify: `src/lib/brain/compiler.ts` — add vocabulary extraction
Modify: `src/lib/brain/types.ts` — add to CharacterBrain
Modify: `src/lib/brain/prompt.ts` — inject into directives
...
function deriveVocabulary(bot: CompilerInput): Vocabulary {
  ...
  const age = parseAgeRange(persona.ageRange);  // "18-24" → 21
...
person structures thoughts as premise→conclusion. An emotional person leads with feelings.
...
|-----------|---------------|---------------|------------|
| **Analytical** | Premise → evidence → conclusion. Notices patterns. Asks "why?" | "wait actually that makes sense because..." | Structured takes, numbered thoughts |
...
This creates a circular problem — the model generates examples that sound like the
...
Modify: `src/lib/ai/caption.ts` → `generateCaption()`
...
1. **Vocabulary fingerprints** (1-2 days) — Highest impact for lowest effort
2. **Interest-based feed filtering** (1 day) — Stops nonsensical cross-niche comments
3. **Temperature differentiation** (few hours) — Small change, meaningful output variation
4. **Cognitive archetypes** (1-2 days) — Changes HOW bots think, not just what they say
5. **Better voice examples** (ongoing) — Start with human curation for seed bots
...
4. If yes → personality system is working
5. If no → the model is still converging
